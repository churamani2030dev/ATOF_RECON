...
public class ZAndPhiForBarsZPositionAndTimePlotterWithDBSCAN {
    ...
    private static final int MAX_COMPONENT = 9;  // Maximum wedge index
    private static final int NUM_BARS = 60;
    private static final int NUM_WEDGES_PER_BAR = 10;
    private static final double WEDGE_SPACING = 30.0; // in mm
    private static final double V_EFFECTIVE = 200.0; // in mm/ns
    private static final double BAR_LENGTH = 280.0; // in mm
    ...
    private static void processEvents(HipoReader reader) {
        ...
        while (reader.hasNext()) {
            ...
            double leftTime = -1.0, rightTime = -1.0;
            int barComponent = -1, barSector = -1;
            boolean barHitDetected = false;
            ...
            for (int hitIndex = 0; hitIndex < numHits; hitIndex++) {
                ...
                if (layer == 0) {  // Bar Hit
                    double phi = 2 * Math.PI * component / NUM_BARS;
                    ...
                    System.out.printf("Bar Hit -> Sector: %d, Layer: %d, Component: %d, Side (Order): %s, ADC: %d, Time: %.2f ns, Pedestal: %d, Phi: %.2f rad\n",
                                      sector, layer, component, (order == 0 ? "Left" : "Right"), adc, time, pedestal, phi);

                    // If both left and right times are recorded, calculate Z and TBar for the bar
                    if (leftTime >= 0 && rightTime >= 0 && barComponent == component && barSector == sector) {
                        double deltaT = rightTime - leftTime;
                        double zBar = (V_EFFECTIVE * deltaT) / 2;
                        double tBar = Math.min(
                            leftTime - (zBar - BAR_LENGTH / 2) / V_EFFECTIVE,
                            rightTime - (BAR_LENGTH / 2 - zBar) / V_EFFECTIVE
                        );

                        System.out.printf("   Bar Z and Single Time -> Z Position: %.2f mm, TBar: %.2f ns\n", zBar, tBar);
                        leftTime = -1.0;  // Reset for the next bar hit
                        rightTime = -1.0;
                    }

                } else if (layer >= 10 && layer <= 19) {  // Wedge Hit
                    int wedgeIndex = component % NUM_WEDGES_PER_BAR;  // Calculate wedge index in each bar
                    double phi = 2 * Math.PI * (component / NUM_WEDGES_PER_BAR) / NUM_BARS;
                    double zWedge = (wedgeIndex - (NUM_WEDGES_PER_BAR - 1) / 2.0) * WEDGE_SPACING;

                    System.out.printf("Wedge Hit -> Sector: %d, Layer: %d, Component (Wedge Index): %d, ADC: %d, Time: %.2f ns, Pedestal: %d, Z Position: %.2f mm, Phi: %.2f rad, Time Wedge: %.2f ns\n",
                                      sector, layer, wedgeIndex, adc, time, pedestal, zWedge, phi, time);
                }
            }
            eventCount++;
        }
