...
public class ZAndPhiForBarsZPositionAndTimePlotterWithDBSCAN {
    ...
    private static final int NUM_BARS = 60;           // Total bars
    private static final int NUM_WEDGES = 10;         // Wedges per bar
    private static final double WEDGE_SPACING = 30.0; // mm
    private static final double V_EFFECTIVE = 200.0;  // mm/ns
    ...
    private static void processEvents(HipoReader reader) {
        ...
        while (reader.hasNext()) {
            ...
            int barLeftTime = -1, barRightTime = -1;
            int barComponent = -1;
            ...
            for (int hitIndex = 0; hitIndex < numHits; hitIndex++) {
                ...
                if (layer == 0) { // Bar hits
                    ...
                    double phi = 2 * Math.PI * barComponent / NUM_BARS;

                    // Print individual bar hit information immediately
                    if (order == 0) { // Left side
                        barLeftTime = (int) time;
                        hasLeft = true;
                        System.out.printf("Bar Hit -> Sector: %d, Layer: %d, Component: %d, Side (Order): Left, ADC: %d, Time: %.2f ns, Pedestal: %d, Phi: %.2f rad%n",
                                          sector, layer, component, adc, time, pedestal, phi);
                    } else if (order == 1) { // Right side
                        barRightTime = (int) time;
                        hasRight = true;
                        System.out.printf("Bar Hit -> Sector: %d, Layer: %d, Component: %d, Side (Order): Right, ADC: %d, Time: %.2f ns, Pedestal: %d, Phi: %.2f rad%n",
                                          sector, layer, component, adc, time, pedestal, phi);
                    }

                    // Once both left and right times are available, calculate Z and TBar
                    if (hasLeft && hasRight) {
                        double zBar = V_EFFECTIVE * (barLeftTime - barRightTime) / 2;
                        double tBar = Math.min(barLeftTime, barRightTime);
                        System.out.printf(" Bar Z and Single Time -> Z Position: %.2f mm, TBar: %.2f ns%n", zBar, tBar);
                    ...
                    double phiWedge = 2 * Math.PI * barComponent / NUM_BARS; // Same Phi as bar

                    // Print wedge hit information
                    System.out.printf("Wedge Hit -> Sector: %d, Layer: %d, Component (Wedge Index): %d, ADC: %d, Time: %.2f ns, Pedestal: %d, Z Position: %.2f mm, Phi: %.2f rad, Time Wedge: %.2f ns%n",
                                      sector, layer, wedgeIndex, adc, time, pedestal, zWedge, phiWedge, time);
                }
            }
            eventCount++;
        }
