...
public class ZAndPhiForBarsZPositionAndTimePlotterWithDBSCAN {
    ...
    private static final int MAX_COMPONENT = 9;  // Maximum allowed component index
    private static final int MAX_LAYER = 3;      // Maximum allowed layer index
    ...
    private static void processEvents(HipoReader reader) {
        ...
        while (reader.hasNext()) {
            ...
            for (int hitIndex = 0; hitIndex < numHits; hitIndex++) {
                ...
                // Apply normalization without printing
                layer = Math.floorMod(layer, MAX_LAYER + 1);
                component = Math.floorMod(component, MAX_COMPONENT + 1);

                // Display the processed hit
                System.out.printf("Processed Hit -> Sector: %d, Layer: %d, Component: %d, Order: %d, ADC: %d, Time: %.2f, Pedestal: %d\n",
                                  sector, layer, component, order, adc, time, pedestal);
            }
            eventCount++;
        }
    }
