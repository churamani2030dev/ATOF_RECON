...
public class ZAndPhiForBarsZPositionAndTimePlotterWithDBSCAN {
    ...
    private static final int NUM_LAYERS = 4;  // Valid layers: 0-3
    ...
    private static class Cluster {
        ...
        public boolean isValidCluster() {
            return events.size() >= 2;  // Fix: At least 2 hits for a valid cluster
        }
    ...
    private static void processEvents(HipoReader reader) {
        ...
        while (reader.hasNext()) {
            ...
            for (int hitIndex = 0; hitIndex < numRows; hitIndex++) {
                ...
                // Fix: Check that the layer is within the valid range 0 to 3
                if (layer < 0 || layer >= NUM_LAYERS) {
                    System.err.println("Invalid layer index: " + layer + ". Ignoring hit.");
                    continue;
                }
                ...
                deltaPhiList.add(Math.abs(phi - phi));  // Since phi for bar and wedge are the same, deltaPhi will be 0
    ...
    // Fix invalid layers (force into valid range 0 to NUM_LAYERS)
    private static int fixLayer(int layer) {
        return Math.min(Math.max(layer, 0), NUM_LAYERS - 1);
    }
    ...
    private static void printClusters(List<Cluster> clusters) {
        int clusterIndex = 1;
        for (Cluster cluster : clusters) {
            if (cluster.isValidCluster()) { // Ensuring only valid clusters with 2+ hits are printed
