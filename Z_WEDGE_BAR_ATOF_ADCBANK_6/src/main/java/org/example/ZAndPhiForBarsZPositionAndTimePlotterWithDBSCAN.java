    ...
    private static void processEvents(HipoReader reader) {
        ...
        while (reader.hasNext()) {
            ...
            for (int hitIndex = 0; hitIndex < numHits; hitIndex++) {
                ...
                if (layer == 0) { // Bar hit
                    phiBar = 2 * Math.PI * component / 60; // Calculate Phi for the bar

                    if (order == 0) { // Left side
                        timeLeft = time; // Capture the raw time directly
                        leftHitFound = true;
                        System.out.printf("Bar Hit -> Sector: %d, Layer: %d, Component: %d, ADC: %d, Time (Left): %.2f ns, Pedestal: %d\n",
                                sector, layer, component, adc, timeLeft, pedestal);
                    } else { // Right side
                        timeRight = time; // Capture the raw time directly
                        rightHitFound = true;
                        System.out.printf("Bar Hit -> Sector: %d, Layer: %d, Component: %d, ADC: %d, Time (Right): %.2f ns, Pedestal: %d\n",
                                sector, layer, component, adc, timeRight, pedestal);
                    }

                    if (leftHitFound && rightHitFound && lastComponent != component) {
                        // Calculate ZBar based on time difference directly
                        double deltaT = timeLeft - timeRight;
                        zBar = VEFF * deltaT / 2;
                        ...
                        double tBarLeft = timeLeft - (zLeft / VEFF);
                        double tBarRight = timeRight - (zRight / VEFF);
                        tBar = Math.min(tBarLeft, tBarRight); // Choosing the minimum time for TBar
                    ...
                    double phiWedge = phiBar; // Phi same as bar
                    ...
                    System.out.printf("Wedge Hit -> Sector: %d, Layer: %d, Component: %d, ADC: %d, Time Wedge: %.2f ns, Pedestal: %d, Z Position: %.2f mm, Phi: %.2f rad\n",
                            sector, layer, wedgeIndex, adc, timeWedge, pedestal, zWedge, phiWedge);

                    // Clustering logic: Check conditions with bar values
