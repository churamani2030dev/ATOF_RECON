...
public class ZAndPhiForBarsZPositionAndTimePlotterWithDBSCAN{
    ...
    public static void main(String[] args) {
        ...
        try (PrintWriter writer = new PrintWriter(new FileWriter("ATOF_ADC_output.txt"))) {
    ...
    private static void processEvents(HipoReader reader, PrintWriter writer) {
        ...
        while (reader.hasNext()) {
            ...
            for (int hitIndex = 0; hitIndex < numHits; hitIndex++) {
                ...
                String output;
                if (layer == 0) {
                    output = String.format("Bar Hit -> Sector: %d, Layer: %d, Component (PMT Position): %d, Side (Order): %s, ADC: %d, Time: %.2f, Pedestal: %d",
                            sector, layer, component, (order == 0 ? "Left" : "Right"), adc, time, pedestal);
                } else if (layer >= 1 && layer <= 3) {
                    output = String.format("Wedge Hit -> Sector: %d, Layer: %d, Component (PMT Position): %d, ADC: %d, Time: %.2f, Pedestal: %d",
                            sector, layer, component, adc, time, pedestal);
                } else {
                    output = String.format("Unknown Hit -> Sector: %d, Layer: %d, Component: %d, Order: %d, ADC: %d, Time: %.2f, Pedestal: %d",
